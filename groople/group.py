...
class Group:
    ...
    def parse_day(d, schedule):
        ...
        if schedule is not None:
            ...
            if m:
                res["order"] += (int(m.group(1)) * 60 + int(m.group(2))) / (24 * 60)
    ...
    def sanitize(self):
        ...
        if 'schedule' in self.attributes:  # pylint: disable=unsupported-membership-test
            s = self.attributes['schedule'].strip() # pylint: disable=unsubscriptable-object
            if not l.endswith(s):
                raise Exception(f"Inconsistent Label : {l} does not end with {s}")
            l = l[:-len(s)].strip()
        else:
            s = None
